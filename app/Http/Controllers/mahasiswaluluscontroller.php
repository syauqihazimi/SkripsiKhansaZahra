...
class mahasiswaluluscontroller extends Controller
{
    ...
    public function jumlah_mahasiswa_kelamin(Request $request){
        ...
        if ($year && $selectedJurusan) {
            $query = DB::table('reg_mahasiswa as r')
                ...
                ->where('r.status', '=', 'Lulus')  // Memfilter berdasarkan status 'Lulus'
                ->groupBy('m.kelamin')
                ->orderBy('m.kelamin')
                ->get();
        }
    
        return view('mahasiswa_lulus.jenis_kelamin', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
    ...
    public function jurusan(Request $request){
        ...
        return view('mahasiswa_lulus.jurusan', compact('years', 'query'));
    }
    ...
    public function jenisseleksi(Request $request){
        ...
        return view('mahasiswa_lulus.jenis_seleksi', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
    ...
    public function jurusansekolah(Request $request){
        ...
        return view('mahasiswa_lulus.jurusan_sekolah', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
    ...
    public function jenissekolah(Request $request){
        ...
        return view('mahasiswa_lulus.jenis_sekolah', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
    ...
    public function nilaibahasa(Request $request)
    {
        ...
        if ($year && $selectedJurusan && $nilaiBahasa) {
            if ($nilaiBahasa == 'etic') {
                $query = DB::table('reg_mahasiswa as r')
                    ...
                    ->join('jurusan as j', 'r.kd_jurusan', '=', 'j.kd_jurusan')
                    ->select(
                        DB::raw('COUNT(CASE WHEN r.etic >= 450 THEN 1 ELSE NULL END) AS jumlah_mahasiswa_lulus'),
                        DB::raw('COUNT(CASE WHEN r.etic <= 449 THEN 1 ELSE NULL END) AS jumlah_mahasiswa_tidak_lulus')
                    )
                    ->whereYear('m.tahun_angkatan', '=', $year)
                    ->where('j.jurusan', '=', $selectedJurusan)
                    ->where('r.status', '=', 'Lulus')  // Memfilter berdasarkan status 'Lulus'
        ...
        return view('mahasiswa_lulus.nilai_bahasa', compact('years', 'allJurusan', 'selectedJurusan', 'nilaiBahasa', 'query'));
    }


    public function tahunlulus(Request $request)
{
    $years = DB::table('mahasiswa as m')
        ->join('reg_mahasiswa as r', 'm.nim', '=', 'r.nim')
        ->select(DB::raw('YEAR(m.tahun_angkatan) as year'))
    ...
    // Jika tahun dan jurusan dipilih, lakukan query untuk mendapatkan jumlah mahasiswa
    if ($year && $selectedJurusan) {
        $query = DB::table('reg_mahasiswa as r')
            ->join('mahasiswa as m', 'r.nim', '=', 'm.nim')
            ->join('jurusan as j', 'r.kd_jurusan', '=', 'j.kd_jurusan')
            ->select(
                DB::raw('YEAR(r.tanggal_lulus) as tahun_kelulusan'),
                DB::raw('COUNT(r.nim) as jumlah_mahasiswa')
            )
            ->whereYear('m.tahun_angkatan', '=', $year)
            ->where('j.jurusan', '=', $selectedJurusan)
            ->where('r.status', '=', 'Lulus')
            ->groupBy(DB::raw('YEAR(r.tanggal_lulus)'))
    ...
    return view('mahasiswa_lulus.tahun_lulus', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
}

public function kategorilulus(Request $request)
{
    ...
    if ($selectedYear && $selectedJurusan) {
        $query = DB::table('reg_mahasiswa as r')
            ...
            ->select(
                DB::raw("
                    CASE
                        WHEN (YEAR(r.tanggal_lulus) - YEAR(m.tahun_angkatan)) BETWEEN 3 AND 4 THEN 'tepat lulus'
                        WHEN (YEAR(r.tanggal_lulus) - YEAR(m.tahun_angkatan)) > 4 THEN 'tidak tepat lulus'
                        ELSE 'belum memenuhi'
                    END AS kategori_lulus
                "),
                DB::raw('COUNT(r.nim) as jumlah_mahasiswa')
            )
            ...
            ->where('j.jurusan', '=', $selectedJurusan)
            ->where('r.status', '=', 'Lulus')
            ->groupBy('kategori_lulus')
            ->orderBy('kategori_lulus')
            ->get();
    }

    // Mengembalikan view dengan data yang diperlukan
    return view('mahasiswa_lulus.kategori_tahun_lulus', compact('years', 'allJurusan', 'selectedYear', 'selectedJurusan', 'query'));
}
...
public function kategoriipk(Request $request)
{
    ...
    // Mengembalikan view dengan data yang diperlukan
    return view('mahasiswa_lulus.indeks_predikat_kumulatif', compact('years', 'allJurusan', 'selectedYear', 'selectedJurusan', 'query'));
}
    ...
    public function kota(Request $request){
        ...
        return view('mahasiswa_lulus.kota', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
    ...
    public function kategorikota(Request $request)
{
    ...
    // Mengembalikan view dengan data yang diperlukan
    return view('mahasiswa_lulus.kategori_kota', compact('years', 'allJurusan', 'selectedYear', 'selectedJurusan', 'query'));
}
    ...
    public function provinsi(Request $request){
        ...
        return view('mahasiswa_lulus.provinsi', compact('years', 'allJurusan', 'selectedJurusan', 'query'));
    }
